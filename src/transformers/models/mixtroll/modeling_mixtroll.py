...
_CONFIG_FOR_DOC = "MixtrollConfig"
...
# Copied from transformers.models.llama.modeling_llama.LlamaRMSNorm with Llama->Mixtroll
class MixtrollRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        MixtrollRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
# Copied from transformers.models.mistral.modeling_mistral.MistralRotaryEmbedding with Mistral->Mixtroll
class MixtrollRotaryEmbedding(nn.Module):
...
# Copied from transformers.models.mistral.modeling_mistral.MistralAttention with Mistral->Mixtroll
class MixtrollAttention(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper. Modified to use sliding window attention: Longformer
    and "Generating Long Sequences with Sparse Transformers".
    """

    def __init__(self, config: MixtrollConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        self.rotary_emb = MixtrollRotaryEmbedding(
            self.head_dim,
...
# Copied from transformers.models.mistral.modeling_mistral.MistralFlashAttention2 with Mistral->Mixtroll
class MixtrollFlashAttention2(MixtrollAttention):
    """
    Mixtroll flash attention module. This module inherits from `MixtrollAttention` as the weights of the module stays
...
# Copied from transformers.models.mistral.modeling_mistral.MistralSdpaAttention with Mistral->Mixtroll
class MixtrollSdpaAttention(MixtrollAttention):
    """
    Mixtroll attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `MixtrollAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from MixtrollAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "MixtrollModel is using MixtrollSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
class MixtrollBlockSparseTop2MLP(nn.Module):
    def __init__(self, config: MixtrollConfig):
        super().__init__()
...
class MixtrollDecoderLayer(nn.Module):
    def __init__(self, config: MixtrollConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.post_attention_layernorm = MixtrollRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class MixtrollPreTrainedModel(PreTrainedModel):
    config_class = MixtrollConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["MixtrollDecoderLayer"]
...
class MixtrollModel(MixtrollPreTrainedModel):
    ...
    def __init__(self, config: MixtrollConfig):
        ...
        self.layers = nn.ModuleList(
            [MixtrollDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self._attn_implementation = config._attn_implementation
        self.norm = MixtrollRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        ...
        if attention_mask is not None and self._attn_implementation == "flash_attention_2" and use_cache:
            ...
            if is_padding_right:
                raise ValueError(
                    ...
                    " this may lead to unexpected behaviour for Flash Attention version of Mixtroll. Make sure to "
...
class MixtrollForCausalLM(MixtrollPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = MixtrollModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, MixtrollForCausalLM

        >>> model = MixtrollForCausalLM.from_pretrained("mistralai/Mixtroll-8x7B-v0.1")
        >>> tokenizer = AutoTokenizer.from_pretrained("mistralai/Mixtroll-8x7B-v0.1")
...
@add_start_docstrings(
    """
    The Mixtroll Model transformer with a sequence classification head on top (linear layer).

    [`MixtrollForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
