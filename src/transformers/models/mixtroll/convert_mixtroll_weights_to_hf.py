...
def write_model(model_path, input_base_path, model_size, safe_serialization=True):
    ...
    print("Loading the checkpoint in a Mixtroll model.")
    with torch.device("meta"):
        model = MixtrollForCausalLM(config)
...
def main():
    ...
    parser.add_argument(
        "--input_dir",
        help="Location of Mixtroll weights, which contains tokenizer.model and model folders",
        required=True,
    )
    parser.add_argument(
        "--model_size",
        choices=["7B"],
        help="'f' models correspond to the finetuned versions, and are specific to the Mixtroll official release. For more details on Mixtroll, checkout the original repo: https://huggingface.co/mistral-ai",
        default="7B",
    )
