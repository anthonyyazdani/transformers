...
@dataclass
class OllCausalLMOutputWithPast(ModelOutput):
    """
    Base class for causal language model (or autoregressive) outputs with past key values, hidden states, and attentions.
    ...
    Args:
        ...
        router_logits (`tuple(torch.FloatTensor)`, *optional*, returned when `output_router_logits=True` is passed or when `config.add_router_probs=True`):
            ...
            Router logits of the encoder model, useful to compute the auxiliary loss and the z_loss for the sparse
            modules.
        rdms (`tuple(torch.FloatTensor)`, *optional*, returned when `output_rdms=True` is passed):
            Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, sequence_length, rdm_size)`.

            Representational Dissimilarity Matrices (RDMs) used for model interpretability.
    """
    loss: Optional[torch.FloatTensor] = None
    logits: torch.FloatTensor = None
    past_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    ...
    aux_loss: torch.FloatTensor = None
    router_logits: Optional[Tuple[torch.FloatTensor]] = None
    rdms: Optional[Tuple[torch.FloatTensor]] = None


@dataclass
class OllModelOutputWithPast(ModelOutput):
    """
    Base class for model's outputs with potential hidden states, attentions, and past key values.
